...
- Use `capture_layout({parentId, problemsOnly: true})` and `capture_screenshot({nodeIds, screenShotDir: "<dir>"})` periodically and at the end to verify design changes (see Tiered Validation).
...
- **Fonts**: `Inter` is the safe **fallback** font. Prefer the distinctive typefaces specified by the active style guide (e.g. `Geist`, `Outfit`, `Cabinet Grotesk`, `Satoshi`) — but only when they are actually available. Always call `get_available_fonts` first; if a preferred font is missing, fall back to a clean system sans-serif (Inter is an acceptable last resort). Never let a missing font abort generation. Always specify `fontName` when creating text.

### Typography Code Example

**Weight rule**: weight is controlled by `fontWeight` as a **numeric string** (`"400"`, `"700"`). Do NOT use word weights like `"bold"`. `fontName.style` (e.g. `"Bold"`) is the *named typographic cut* of the family and may also be required to select the right variant — if both are present, `fontWeight` is authoritative for the rendered weight.

```javascript
title=I("parent", {type: "text", name: "Page Title", content: "Welcome", fontSize: 32, fontName: {family: "Geist", style: "Bold"}, fontWeight: "700", fill: "#18191C", textAlignHorizontal: "LEFT", textAutoResize: "HEIGHT", width: "fill_container"})
```
...
Bind reusable design tokens to node properties with the **`$:<SetName>:<VariableName>`** syntax. The `SetName` is the variable set (collection) name and `VariableName` is the variable name within that set. Use `fetch_variables` to discover available variable sets, or `apply_variables` to create/update variables. Variable types: `FLOAT`, `COLOR`, `BOOLEAN`, `STRING`.
...
title=I(card, {type: "text", content: "$:Content:app-title", fontSize: "$:Primitives:heading-size", fontName: "$:Primitives:body-font", fill: "$:Semantic:text-primary"})
```
...
- Type must match: FLOAT for number properties, COLOR for fill/stroke, BOOLEAN for visible, STRING for content/fontName.
...
- 图片生成用 `G()` 操作，`type` 取 `"stock"`（真实图库随机图，首选）/ `"ai"`（按 prompt 生成）/ `"placeholder"`（灰底标签占位框，用于快速草稿）。
...
- Always `capture_screenshot({nodeIds: [iconId], screenShotDir: "<dir>"})` after creating SVG icons to verify.
...
- Set `placeholder: true` to mark a frame as a placeholder container (e.g. screenshot/section stubs) — a frame boolean property (accepted at runtime though not listed in the published Node Property Schema). Distinct from the G() `"placeholder"` image type, which fills a node with a gray label box.
...
| Font different | Unavailable style | Use the available style name from `get_available_fonts` (e.g. "Regular", "Medium", "Bold"); fall back to Inter if preferred fonts are missing |
...
|---|---|---|
| **T1 — Structural scaffold** | New frames, layout mode, padding, hierarchy | `capture_layout({parentId, problemsOnly: true})` only — screenshot not useful yet |
| **T2 — Content fill** | Text content, token binding, component instance props | **Skip validation**; defer to the next style/phase batch |
| **T3 — Visual/style** | `fill`, typography, effects, cornerRadius, strokes | `capture_screenshot({nodeIds, screenShotDir: "<dir>"})` only |
| **T4 — Section complete** | A whole logical section (hero, features, footer) is done | Run **both** `capture_screenshot({nodeIds, screenShotDir: "<dir>"})` + `capture_layout({parentId, problemsOnly: true})` **once** |
| **T5 — Final page** | All sections merged | One final `capture_screenshot({nodeIds: [pageId], screenShotDir: "<dir>"})` of the full page |
...
- `capture_screenshot({nodeIds, screenShotDir: "<dir>"})` for nodes > 2000px tall: screenshot sections, not the whole node.
