...
* **ANTI-SLOP:** Avoid `Inter` as a *first choice* for "Premium" or "Creative" vibes. Prefer distinctive typefaces like `Geist`, `Outfit`, `Cabinet Grotesk`, or `Satoshi`. `Inter` is the acceptable **fallback** when those are unavailable (see font-availability rule below).
...
* **Font availability fallback:** The bans above assume distinctive typefaces (Geist, Outfit, Cabinet Grotesk, Satoshi) are installed in Ardot. If a recommended font is unavailable, call `get_available_fonts` and fall back to a clean system sans-serif — including Inter as a last resort. Never let a missing font abort generation; degrade gracefully while staying as close to the intended aesthetic as possible.
...
### Typography
* **AVOID Inter as first choice** — prefer `Geist`, `Outfit`, `Cabinet Grotesk`, or `Satoshi`. `Inter` is only acceptable as a fallback when the preferred fonts are unavailable (per `get_available_fonts`); never treat it as the default premium choice.
...
### External Resources
* **NO External Image URLs** — Ardot has no `image` node type; images are fills on a frame/rectangle node. Always generate imagery with the `G()` operation (`type: "stock"` preferred real-library image, `"ai"` for prompt-generated, or `"placeholder"` for a gray labeled stub). Never paste external URLs (Unsplash, picsum.photos, etc.) — they do not work as fills and violate the platform model.
