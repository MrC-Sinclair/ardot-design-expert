...
> **双证据硬门（与 `code-review-workflow.md` 同标准）**：本流程所有「设计稿无问题 / 质量达标」结论，必须同时基于 ① `batch_read` 节点声明值（开启 `resolveVariables`）+ ② `capture_screenshot` 实际渲染像素。**无视觉能力时一律改用 Pillow 程序化取色兜底，禁止只报节点数据**。仅依赖截图肉眼、或仅依赖节点 JSON，均视为证据不足、不得下终论。像素与声明冲突时以像素为准；仅采到单源须显式标「低置信度」。
...
- 仅报告工具 / 数据实际反映的问题。
- **双证据缺失即降级**：未能同时取得节点声明值与实际渲染像素时，结论须标「低置信度、视觉未验证」，不得写「已确认无问题」。

> **相关流程**：本流程审「设计稿本身」质量；若需审「代码还原设计稿的还原度」或「设计稿对齐真实代码（改设计贴代码）」，转 `references/code-review-workflow.md`（双向校对：模式 A / 模式 B）。三者共用同一条双证据硬门。
