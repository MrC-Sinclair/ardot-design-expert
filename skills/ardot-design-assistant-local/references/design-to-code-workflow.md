...
```
capture_screenshot(nodeIds: ["<frameId>", ...], screenShotDir: "<dir>") → get visual references for all pages in one call
```
...
Start a local preview server and open it in the IDE's built-in browser, then compare page-by-page against design screenshots. Use `start_preview_server` (from the frontend_tools MCP) to serve the project, or fall back to `python3 -m http.server <port>` in `<project>` and open the returned URL:
...
- Text node and frame node `batch_read(..., properties: [...])` reads in parallel
...
### Box Model & Overflow Pitfall (critical)

Any element with `width: 100%` / `max-width` plus `padding` or `border` must declare `box-sizing: border-box;`. Otherwise the rendered width equals the specified width plus padding plus border, which overflows the parent container — typically surfacing as a **horizontal scrollbar** on the page or clipped child elements.

- **Preferred:** add a one-time global fallback at the global stylesheet entry point:
  ```css
  *, *::before, *::after { box-sizing: border-box; }
  ```
- If you cannot control the global stylesheet, declare `box-sizing: border-box;` explicitly on every container that carries padding/border.
- When a child centers itself with `max-width` (e.g. `margin: 0 auto`) and the parent also adds left/right padding, the parent must use `border-box`; otherwise the combined width exceeds 100% and overflows.
...
| Text styles don't match | 未读取完整文本属性 | 用 `batch_read(nodeIds: [...], properties: ["characters","fontSize","fontName","fontWeight","lineHeight","letterSpacing","textAlignHorizontal","fills"])` 重新读取文本节点 |
...
| Horizontal scrollbar / container overflows viewport | `width:100%` + `padding` without `box-sizing:border-box`; or child `max-width` + parent padding sum > 100% | Add `box-sizing:border-box` to the container; verify child width + parent padding ≤ viewport width |
